...
//    self.preferredFramesPerSecond = 60;
    ...
    UIImage * myImage = self.rippleImage;
...
-(void)timerFired:(NSTimer *)timer{
    ...
    _timerindex ++;
    CGPoint location = CGPointMake(130, 220);
...
- (void)update
{
    ...
    if (self.rippleImage == nil) {
        return;
    }
  
    if (!_ripple)
    {
        [self loadImageIntoPond];
    }
    ...
    //glViewport(0, 0, self.view.bounds.size.width*2, self.view.bounds.size.height);
...
-(void)loadImageIntoPond
{
  ...
  // 1
  UIImage * image = self.rippleImage;
